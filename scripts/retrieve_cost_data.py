...
########## add pandas to load the file and include modifications on-the-fly
import pandas as pd
...
if __name__ == "__main__":
    ...
    if snakemake.params.costs_update:

        print(f'########## pypsa-es [retrieve_cost_data.py]: Including cost updates detailed in {snakemake.params.costs_update_file}')
        ########## "to_fn" contains the file, e.g. resources/costs_2030.csv
        df = pd.read_csv(to_fn)
        original_columns = df.columns.tolist()

        ########## Load the updates
        df_updates = pd.read_csv(snakemake.params.costs_update_file)
